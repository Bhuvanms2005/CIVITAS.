...
const ComplaintManagementView = ({ user }) => {
  ...
  if (loading) {
    return <div className={styles.container}><p className={styles.loading}>Loading all complaints...</p></div>;
  }
      ...
      {complaints.length === 0 ? (
        ...
        <ul className={styles.complaintsList}>
          {complaints.map(complaint => (
            <li key={complaint._id} className={styles.complaintItem} onClick={() => handleOpenModal(complaint._id)}>
              <div className={styles.itemHeader}>
                <h3 className={styles.itemTitle}>{complaint.subType} ({complaint.category})</h3>
              ...
              <p className={styles.submittedBy}>Submitted by: {complaint.submittedBy?.firstName} {complaint.submittedBy?.lastName} ({new Date(complaint.submittedAt).toLocaleString()})</p>
              {complaint.assignedTo && (
                <p className={styles.assignedTo}>Assigned to: {complaint.assignedTo.firstName} {complaint.assignedTo.lastName}</p>
              )}
              {complaint.photo && (
              ...
              <div className={styles.itemActions}>
                <button className={styles.actionButton} onClick={(e) => { e.stopPropagation(); handleOpenModal(complaint._id); }}>View Details</button>
                <button className={`${styles.actionButton} ${styles.deleteButton}`} onClick={(e) => { e.stopPropagation(); alert(`Delete complaint ${complaint._id}`); }}>Delete</button>
              </div>
            </li>
          ))}
        </ul>
